...
public class Frame extends JFrame implements ActionListener{
		...
		private static String home_directory = "C:/Documents and Settings/Gonzalo/workspace/";
		...
		public Frame(Client c) throws IOException {
			...
			selected_register.setLabel("Seleccionado: Ninguno");
					...
					selection = JOptionPane.showConfirmDialog(
					 ...
					 if (selection == JOptionPane.OK_OPTION) {
						 ...
						 for(int i = 0; i < temp.length; i++){
							...
							params[1] += i != temp.length-1 ? config[i] + "," : config[i]; 
		...
		private void setRegister(String[] temp, JComponent[] context, int length) throws IOException{
			if(table_name.isEmpty()){
				...
				String str = Log.readFile(home_directory + "params.txt");
	        	String str2 = "";
	        	int[] k = new int[temp.length]; 
				for(int i = 0; i < temp.length; i++){	
					k[i] = str.indexOf(temp[i]);		
        		}
				Arrays.sort(k);
				for(int i = 0; i < temp.length; i++){				
        			str2 += str.substring(str.indexOf("-", k[i]-3), str.indexOf("]", k[i])+2);     		
        		}
        		String[] hola = str2.split("]");
        		for(int i = 0; i < temp.length; i++){
        			str2 = hola[i].substring(hola[i].indexOf("[")+1, hola[i].lastIndexOf("-")+11);
        			if(table_name.equals(hola[i].substring(hola[i].indexOf("-")+2, hola[i].lastIndexOf("-")-1))){
        				for(int j = 0; j < length; j++){	
        					initialValues[i][j] = str2.charAt(2*j)-48;
